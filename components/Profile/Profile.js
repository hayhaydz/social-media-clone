...
const Profile = ({ jwt, currentUsersID, query_id, message, setMessage }) => {
    ...
    const url = `${process.env.PRIVATE_API_URL}/api/user/${query_id}`;
                ...
                <>
                    {response.status !== 'fail' &&
                        <>
                            ...
                            <ViewPosts jwt={jwt} currentUsersID={currentUsersID} message={message} setMessage={setMessage} url={`${process.env.PRIVATE_API_URL}/api/post/u/${response.data.username}`} />
                        </>
                    }
                </>
            }
